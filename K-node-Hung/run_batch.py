...
def main():
    ...
    parser.add_argument("-c", "--cpus-per-task",
                        help="The number of CPUs to use for each task. Default is 1. This only affect in SLURM mode.",
                        type=int,
                        default=1)
    ...
    for k in range(1, maxK + 1):
        ...
        for experiment in range(1, args.experiments + 1):
            ...
            if args.no_slurm:
                python_script = f"python run.py {network_name} {polarization_fn_name} {
                    k} {k} {experiment} -r {game_rounds} -m {memory} {zero_sum_argument}"
